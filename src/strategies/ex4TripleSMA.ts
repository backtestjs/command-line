...
export async function runStrategy(bth: BTH) {
  ...
  if (position === 'low' && (lowSMA > midSMA || lowSMA > highSMA)) {
    ...
    if (timesBought === 1 && lowSMA > midSMA && lowSMA > highSMA) {
      await bth.buy({ amount: bth.orderBook.preBoughtQuoteAmount * 0.25 })
      timesBought++
    }

    // Buy with 15% if mid sma crosses high sma
    if (!bonusBuy && midSMA > highSMA) {
      await bth.buy({ amount: bth.orderBook.preBoughtQuoteAmount * 0.15 })
      bonusBuy = true
    }
  }
  ...
  // Buy Short Scenarios
    ...
    // Buy with 25% if low sma crosses mid and high sma
    if (timesBought === 1 && lowSMA < midSMA && lowSMA < highSMA) {
      await bth.buy({ position: 'short', amount: bth.orderBook.preBoughtQuoteAmount * 0.25 })
      timesBought++
    }

    // Buy with 15% if mid sma crosses high sma
    if (!bonusBuy && midSMA < highSMA) {
      await bth.buy({ position: 'short', amount: bth.orderBook.preBoughtQuoteAmount * 0.15 })
      bonusBuy = true
    }
  }
